...
不需要 VPN，不需要公网 IP，只要服务器能访问 `open.feishu.cn`，就能在飞书对话框里直接操作内网服务器的 Shell。机器人运行在服务器上，你在飞书里发送命令，执行结果实时推送到消息卡片；文件传输、远端编辑同样在对话框内完成，无需额外工具。

## 能做什么

在飞书对话框输入以下命令，结果几秒内以卡片形式呈现：

```
df -h
systemctl status nginx
tail -f /var/log/app/error.log   # 输出持续滚动，随时点卡片上的 Ctrl+C 中断
```
...
| 操作 | 触发方式 |
|---|---|
| Shell 命令执行 | 直接发送命令文本，输出实时更新到卡片 |
| Ctrl+C 中断 | 点击卡片上的「Ctrl+C」按钮，终止运行中的命令 |
| 文件下载 | `/get <文件或目录>`，目录自动打包为 zip 发回对话框 |
| 文件上传 | 拖拽文件到对话框，点击卡片「保存」按钮选择服务器目标路径 |
| 远端文件编辑 | `/edit <文件>`，配合本地 `larksh-client` 用 `$EDITOR` 编辑后自动写回 |
| 会话状态保持 | 每个用户/群聊独立 PTY 会话，工作目录、环境变量跨命令保持 |
| 安全管控 | 用户白名单、命令黑名单、全量审计日志（JSONL 格式） |
